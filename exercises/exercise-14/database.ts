...
type Option<T> = Partial<SortOption<T> & ProjectionOption<T>>
...
export class Database<T> {
    ...
    protected handleOption(input: T[], option: Option<T>): Partial<T>[] {
        let result = input as Partial<T>[];
    ...
    async find(query: Expression<T>, option: Option<T>): Promise<Partial<T>[]> {
