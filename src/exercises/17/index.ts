...
export function assertUnreachable(value: never): never {
    // Implement this in the solution
    throw new Error(`Unexpected value: ${value}`);
}
...
export function getUserLabel(person: Person): string {
    switch (person.type) {
        ...
        // Missing moderator case - add exhaustiveness check!
    }
    // Add exhaustiveness check here
    return '';
}
...
export function processStatus(status: Status): string {
    switch (status) {
        ...
        // Missing 'banned' case - add it and exhaustiveness check!
    }
    // Add exhaustiveness check here
    return '';
}
